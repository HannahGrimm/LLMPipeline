...
(assert (distinct sort_int sort_Field sort_Null sort_Heap sort_java.lang.Object sort_any sort_boolean))
...
; --- Sequent
(assert (not (=> (and (and (and (and (and (and (and (and (and (>= (u2i u_N) (* (u2i u_n) (u2i (|u_Helper::pow| u_heap (i2u 2) u_l)))) (< (u2i u_N) (* (+ (u2i u_n) 1) (u2i (|u_Helper::pow| u_heap (i2u 2) u_l))))) (not (= u_n (i2u 1)))) (> (u2i u_N) 0)) (<= (u2i u_n) (u2i u_N))) (>= (u2i u_n) 1)) (>= (u2i u_l) 0)) (< (u2i u_l) (u2i u_N))) (< (u2i u_N) 20)) (k_wellFormed u_heap)) (u2b (let ((u_heapAtPre u_heap) (u_N_old u_N)) unknown_0)))))

(check-sat)

; --- Translation of unknown values
; unknown_0 :  \<{  if (l > 0) {    if (N < (2 * n + 1) * Helper.pow(2, l - 1)) {      n = 2 * n;    } else {      n = 2 * n + 1;    }    l = l - 1;  }  ;}\> (and(and(geq(N,mul(n,Helper::pow(heap,Z(2(#)),l))),lt(N,mul(add(n,Z(1(#))),Helper::pow(heap,Z(2(#)),l)))),equals(N,N_old)))
